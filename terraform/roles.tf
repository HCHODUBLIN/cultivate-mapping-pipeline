...
resource "snowflake_grant_privileges_to_account_role" "reader_warehouse" {
  account_role_name = snowflake_role.reader.name
  privileges        = ["USAGE"]
...
resource "snowflake_grant_privileges_to_account_role" "transformer_raw_read" {
  ...
  on_schema {
    schema_name = "\"${snowflake_database.cultivate.name}\".\"${snowflake_schema.raw.name}\""
  }
}
...
resource "snowflake_grant_privileges_to_account_role" "transformer_raw_tables" {
  ...
  on_schema_object {
    all {
      ...
      in_schema          = "\"${snowflake_database.cultivate.name}\".\"${snowflake_schema.raw.name}\""
    }
  }
}
...
resource "snowflake_grant_privileges_to_account_role" "transformer_staging" {
  ...
  on_schema {
    schema_name = "\"${snowflake_database.cultivate.name}\".\"${snowflake_schema.staging.name}\""
  }
}
...
resource "snowflake_grant_privileges_to_account_role" "transformer_intermediate" {
  ...
  on_schema {
    schema_name = "\"${snowflake_database.cultivate.name}\".\"${snowflake_schema.intermediate.name}\""
  }
}
...
resource "snowflake_grant_privileges_to_account_role" "transformer_marts" {
  ...
  on_schema {
    schema_name = "\"${snowflake_database.cultivate.name}\".\"${snowflake_schema.marts.name}\""
  }
}

# ──────────────────────────────────────────────
# Schema Grants — Reader (marts only)
# ──────────────────────────────────────────────

resource "snowflake_grant_privileges_to_account_role" "reader_marts_schema" {
  account_role_name = snowflake_role.reader.name
  privileges        = ["USAGE"]
...
resource "snowflake_grant_privileges_to_account_role" "reader_marts_tables" {
  ...
  on_schema_object {
    all {
      object_type_plural = "TABLES"
      in_schema          = "\"${snowflake_database.cultivate.name}\".\"${snowflake_schema.marts.name}\""
    }
  }
}

# ──────────────────────────────────────────────
# Schema Grants — Loader (raw only)
# ──────────────────────────────────────────────
